/* 
   ...
   runs as "./a.out -L <sidelength(20)> -T <temperature(1.0)> \
   ...
   For compilation, use
   gcc -Wall -I/home/common/studvsmks{XX}/gsl/include -c mc.c; 
   gcc -L/home/common/studvsmks{XX}/gsl/lib mc.o -lgsl -lgslcblas -lm

   If the program is executed on the MIPT cluster, use
   the following command to run the simulation:
   "sbatch -J monte --comment="monte" --partition=RT_study \
   -N 1 --ntasks-per-node=1 --wrap="./a.out -T 10.0""
...
//	  L,T,nCycles,fSamp,Seed);
 ...
 // fprintf(stdout,"# Program ends.\n");
  fprintf(stdout, "%i\n", L*L);
  fprintf(stdout, "ITEM: ATOMS id x y z\n");
  for (i=0;i<L;i++){
    for (j=0;j<L;j++){
      if(i == j == L-1){
        fprintf(stdout, "%i %i %i %i", F[i][j]+2, i, j, 0);
      }
      else{
