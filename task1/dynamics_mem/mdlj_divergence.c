...
/* Writes the data for dynamical memory time. */

void div_out (FILE * fp, 
	      double time, double div_coord, double div_vel) {

  fprintf(fp,"%.8lf %.8lf %.8lf \n", time, div_coord, div_vel);
}
...
int main ( int argc, char * argv[] ) {
  ...
  int N=216;
  double L=0.0;
  double rho=0.5, T=0.0, rc2 = 1.e20, vir, vir_old, pcor, V;
  ...
  int * ix1, * iy1, * iz1;
  int k;
  double vir1, vir_old1;
  double PE1, KE1, TE1, TE01;
  double dt1, dt12;
  double ratio=5.0;
  ...
  for (i=1;i<argc;i++) {
    ...
    else if (!strcmp(argv[i],"-ratio")) ratio=atof(argv[++i]);
  ...
  /* compute the second timestep using the value of ratio */
  dt1 = dt/ratio;
  dt12 = dt1*dt1;
  ...
  ix1 = (int*)malloc(N*sizeof(int));
  iy1 = (int*)malloc(N*sizeof(int));
  iz1 = (int*)malloc(N*sizeof(int));
  ...
  sprintf(fn_div, "%i.txt", (int)ratio);
  out_div=fopen(fn_div,"w");

  PE = total_e(rx,ry,rz,fx,fy,fz,N,L,rc2,ecor,ecut,&vir_old);
  TE0=PE+KE;
  PE1 = total_e(rx1, ry1, rz1, fx1, fy1, fz1, N, L, rc2, ecor, ecut, &vir_old1);
  TE01 = PE1 + KE1;
  ...
  for (s=0;s<nSteps;s++) {
    ...
    for (k=0; k<(int)ratio; k++) {
            /* First integration half-step */
            for (i=0;i<N;i++) {
              rx1[i]+=vx1[i]*dt1+0.5*dt12*fx1[i];
              ry1[i]+=vy1[i]*dt1+0.5*dt12*fy1[i];
              rz1[i]+=vz1[i]*dt1+0.5*dt12*fz1[i];
              vx1[i]+=0.5*dt1*fx1[i];
              vy1[i]+=0.5*dt1*fy1[i];
              vz1[i]+=0.5*dt1*fz1[i];
              /* Apply periodic boundary conditions */
              if (rx1[i]<0.0) { rx1[i]+=L; ix1[i]--; }
              if (rx1[i]>L)   { rx1[i]-=L; ix1[i]++; }
              if (ry1[i]<0.0) { ry1[i]+=L; iy1[i]--; }
              if (ry1[i]>L)   { ry1[i]-=L; iy1[i]++; }
              if (rz1[i]<0.0) { rz1[i]+=L; iz1[i]--; }
              if (rz1[i]>L)   { rz1[i]-=L; iz1[i]++; }
            }
            /* Calculate forces */
            PE1 = total_e(rx1,ry1,rz1,fx1,fy1,fz1,N,L,rc2,ecor,ecut,&vir1);
    ...
    if (s%100==0) {
        ...
        double div_coord = 0.0;
        double div_vel = 0.0;

        for (i=0; i<N; i++) {
            div_coord += (rx[i] + ix[i]*L - rx1[i] - ix1[i]*L)*(rx[i] + ix[i]*L - rx1[i] - ix1[i]*L) + (ry[i] + iy[i]*L - ry1[i] -iy1[i]*L)*(ry[i] + iy[i]*L - ry1[i] -iy1[i]*L) + (rz[i] + iz[i]*L - rz1[i] -iz1[i]*L)*(rz[i] + iz[i]*L - rz1[i] -iz1[i]*L);
            div_vel += (vx[i] - vx1[i])*(vx[i] - vx1[i]) + (vy[i] - vy1[i])*(vy[i] - vy1[i]) + (vz[i] - vz1[i])*(vz[i] - vz1[i]);
        }

        div_coord /= N;
        div_vel /= N;

        div_out(out_div, s*dt, div_coord, div_vel);
